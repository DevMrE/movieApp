        ...
        multiCatch(
            tryBlock = {
                val response = httpClient.get(
                    resource = TrendingRequestDto(
                        trendingRequestType = TrendingRequestTypeDto.ALL
                    )
                )
    ...
    override suspend fun fetchMovies(): Result<ApiResponseDto<TrendingResultDto>, ApiError> =
        multiCatch(
            tryBlock = {
                val response = httpClient.get(
                    resource = TrendingRequestDto(
                        trendingRequestType = TrendingRequestTypeDto.MOVIE
                    )
                )
    ...
    override suspend fun fetchSeries(): Result<ApiResponseDto<TrendingResultDto>, ApiError> =
        multiCatch(
            tryBlock = {
                val response = httpClient.get(
                    resource = TrendingRequestDto(
                        trendingRequestType = TrendingRequestTypeDto.SERIES
                    )
                )
    ...
    override suspend fun fetchPeople(): Result<ApiResponseDto<TrendingResultDto>, ApiError> =
        multiCatch(
            tryBlock = {
                val response = httpClient.get(
                    resource = TrendingRequestDto(
                        trendingRequestType = TrendingRequestTypeDto.PEOPLE
                    )
                )
